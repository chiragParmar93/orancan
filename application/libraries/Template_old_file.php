 ...
 *
 * Additions:
 * Add special methods for adding css and js
 *
...
class Template
{
	...
	// used for metadata in foot mainly for js
	private $_metadata_foot = array();
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['metadata_foot']	= implode("\n\t\t", $this->_metadata_foot);
	...
	/**
	 * prepend javascipt at provided loc
	 *
	 * @return void
	 */
	public function prepend_js($link, $loc = 'foot')
	{
		$script = '<script src="' . site_url($link) . '"></script>';
		
		($loc == 'foot') ? array_unshift($this->_metadata_foot, $script) 
							: array_unshift($this->_metadata, $script);
	...
	/**
	 * append javascipt at provided loc
	 *
	 * @return void
	 */
	public function append_js($link, $loc = 'foot')
	{
		$script = '<script src="' . site_url($link) . '"></script>';
		($loc == 'foot') ? $this->_metadata_foot[] = $script 
							: $this->_metadata[] = $script;
	 ...
	 * @return void
	 */
	public function prepend_css($link)
	{
		$link = '<link rel="stylesheet" href="' . site_url($link) . '">';
	 ...
	 * @return void
	 */
	public function append_css($link)
	{
		$link = '<link rel="stylesheet" href="' . site_url($link) . '">';
