...
class Users extends Admin_Controller {
	...
	public function index()
	{
        ...
        else
        {
            ...
            #$this->load->view('admin/users/index', $this->data, false);
        }
	}

        public function ajaxData(){
            $users=new Users_model;
            $totalRec = count($users->get_users());
            $page = isset($_POST['iDisplayStart']) ? intval($_POST['iDisplayStart']) : 0;
            $rows = isset($_POST['iDisplayLength']) ? intval($_POST['iDisplayLength']) : 1;
            $sort = isset($_POST["iSortCol_0"]) ? $_POST["iSortCol_0"] : NULL;
            ...
            $list = $users->get_datatables($searchArray, 'listing');
            #pr($list,1);
            $totalRow = $users->get_datatables($searchArray, 'count');
            ...
            if ($list) {
                ...
                foreach ($list as $person) {
                    ...
                    $operations = '<div class="dropdown pull-right">
                    ...
                    $operations .= '<li><a href="javascript:void(0)" onclick="Edit_user(' . "'" . encrypt_val($person['id']) . "'" . ')"><i class="fa fa-pencil"></i> Edit user</a></li>
                                    <li><a href="javascript:void(0)" onclick="Delete_user(' . "'" . encrypt_val($person['id']) . "'" . ')"><i class="fa fa-trash"></i> Delete</a></li>';
                    $operations .= '</ul></div>';

                    $data[] = array($first_name, $last_name, $operations);
                    $v++;
                }
            } else {
