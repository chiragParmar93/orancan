 ...
 *
 * Additions:
 * Add my Alert_Model and sync methods
 *
...
class MY_Model extends CI_Model
{
    ...
    /**
     * Try to autoload models
     * @link: https://github.com/bcit-ci/CodeIgniter/issues/3137
    ...
    public function select($select)
    {
        ...
        {
            // single select
            $this->db->select($this->_table . '.' . $select);
        }
        return $this;
    }
    ...
    /**
     * sync multiple records and return their ids
     */
    public function sync_multiple($get_by, $arr)
    {
        $ids = array();
        foreach ($arr as $data)
        {
            $ids[] = $this->sync($get_by, $data);
        }
        return $ids;
    }

    /**
     * sync multiple for single field
     * eg. $field = `interest` and $values = $_POST['interests']
     */
    public function sync_multiple_for($field, $values)
    {
        $ids = array();
        foreach ($values as $value)
        {
            $ids[] = $this->sync(array($field => $value));
        }
        return $ids;
    }
    ...
    /**
     * Returns an array for column
     */
    public function get_array($for)
    {
        $all = $this->get_all();
